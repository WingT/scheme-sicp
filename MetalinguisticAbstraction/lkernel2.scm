...
;4.31
(define (list-of-arg-values exps arg-types env)
  (cond ((no-operands? exps) '())
        ((eq? (car arg-types) 'normal)
         (cons (actual-value (first-operand exps) env)
               (list-of-arg-values
                (cdr exps) (cdr arg-types) env)))
        ((eq? (car arg-types) 'lazy-memo)
         (cons (delay-it (first-operand exps) env)
               (list-of-arg-values
                (cdr exps) (cdr arg-types) env)))
        ((eq? (car arg-types) 'lazy)
         (cons (delay-it-no-memo (first-operand exps) env)
               (list-of-arg-values
                (cdr exps) (cdr arg-types) env)))))
(define (force-it obj)
  (cond ((thunk? obj)
...
(define (true? x)
  (not (eq? x false)))
;4.31
(define (make-procedure parameters-with-types body env)
  (list 'procedure
...
(define (user-print object)
  (if (compound-procedure? object)
      (display (list 'compound-procedure
                     (map (lambda (x y) (if (eq? y 'normal)
                                            x
                                            (list x y)))
                          (procedure-parameters object)
                          (procedure-parameter-types object))
